...
Daraus kann man eine Funktion der maximalen Fl\"ache $A_{max}(c)$ des rechtecks $a \times c$ konstruieren:

\[ A_{max}(c) = a c = 2c(\sqrt{2}-c) \]
...
\subsection{Rechteck}
Was ist das größte Rechteck, was man durch den Gang schieben kann? Ohne drehen ist es natürlich ein $1\times 1$ Quadrat. Mit drehen ist wieder die Funktion $A_{max}(a)$ hilfreich. Sie beschreibt das Rechteck der größten Fläche was noch durch passt, abhängig von einer festen Seitenlänge $a$:

\[ A_{max}(a) = 2a(\sqrt{2}-a) \]
...
\[ A_{max}'(a) = 2(\sqrt{2}-2a) \]

Um dann davon die Nullstelle bei $a = \frac{1}{\sqrt{2}}$ zu finden. Hier ist das größte Rechteck, mit einer Fläche von
